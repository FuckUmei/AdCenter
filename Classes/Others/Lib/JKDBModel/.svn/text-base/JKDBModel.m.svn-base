...
//        MLog(@" 属性名：%@ ",propertyName);
        [proNames addObject:propertyName];
        //获取属性类型等参数
        NSString *propertyType = [NSString stringWithCString: property_getAttributes(property) encoding:NSUTF8StringEncoding];
//        MLog(@"  @1      name:%@   type:%@ ",propertyName,propertyType);
        /*
         ...
         */
        if ([propertyType hasPrefix:@"T@"]&&[propertyType hasPrefix:@"NSDictionary"]) {
...
// 查找未删除的数据
+(NSArray *)findAllNoDeleted{
    NSString *condition = [NSString stringWithFormat:@"WHERE %@=%d",@"IsDel",0];
    return [self findFirstByCriteria:condition];
}
// 查找已经删除的数据
+(NSArray *)findAllYESDeleted{
    NSString *condition = [NSString stringWithFormat:@"WHERE %@=%d",@"IsDel",1];
    return [self findFirstByCriteria:condition];
}
...
//    MLog(@" name = :%@   type = :%@ ",proNames,proTypes);
